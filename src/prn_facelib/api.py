...
from prn_facelib.predictor import PosPrediction
...
class PRN:
    ...
    def process(self, input, image_info = None, do_post_smooth = True, smooth_gamma = 0.7):
        ...
        # import matplotlib.pyplot as plt
        # test = cropped_pos.copy()
        # test_1 = gaussian(test, 1)
        # plt.subplot(121)
        # plt.imshow(test/test.max())
        # plt.subplot(122)
        # plt.imshow(test_1/test_1.max())
        # plt.show()

        #Khanh: add to smooth the zig-zag artifiacts in the final mesh
        if do_post_smooth:
            cropped_pos = gaussian(cropped_pos, smooth_gamma)
