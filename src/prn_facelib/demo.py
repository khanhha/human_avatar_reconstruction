...
from prn_facelib.utils.estimate_pose import estimate_pose
from prn_facelib.utils.rotate_vertices import frontalize
from prn_facelib.utils.render_app import get_visibility, get_uv_mask, get_depth_image
from prn_facelib.utils.write import write_obj_with_colors, write_obj_with_texture

def main(args):
    if args.isShow or args.isTexture:
        import cv2
        from prn_facelib.utils.cv_plot import plot_kpt, plot_vertices, plot_pose_box
    ...
    for i, image_path in enumerate(image_path_list):
        ...
        if 'MVIMG_20190604_180645' not in image_path:
             continue
        ...
        if args.is3d or args.isMat or args.isPose or args.isShow:
            ...
            #khanh
            #save_vertices[:, 0] = w - 1 - save_vertices[:, 0]
        ...
        if args.is3d:
            ...
            if args.isTexture:
                ...
                #khanh
                texture[prn.uv_kpt_ind[1,:], prn.uv_kpt_ind[0,:], :] = (1.0, 0.0, 0.0)
                ...
                img_kpt = np.vstack([img_face_points[prn.uv_kpt_ind[1,:], prn.uv_kpt_ind[0,:],0], img_face_points[prn.uv_kpt_ind[1,:], prn.uv_kpt_ind[0,:],1]]).T
                img_kpt_path = os.path.join(save_folder, name + '_img_kpt.txt')
                np.savetxt(fname=img_kpt_path, X=img_kpt)
                # image[img_kpt[:,1], img_kpt[:,0], :] = (1.0, 0.0, 0.0)
                # import matplotlib.pyplot as plt
                # plt.imshow(image)
                # plt.show()
        ...
        if args.isKpt or args.isShow:
            ...
            #khanh
            #TODO: this must be in line with transformation on save_vertices
            kpt[:,1] = h - 1 - kpt[:,1]
