...
def multiResolution_textureSynthesis(parms, userExample=None):
    ...
    if userExample is not None:
        ...
        # copy to level 0
        canvas.pyramid[0], filledMap.pyramid[0] = copyMap2Map(exampleMap.pyramid[0], canvas.pyramid[0],
                                                              filledMap.pyramid[0])
    ...
    for pLvl in range(0, canvas.levels + 1):
        ...
        if parms["pyramidType"] == "gaussian":
            if pLvl + 1 <= canvas.levels:
                canvas.pyramid[pLvl + 1] = visualizeNextLevel(canvas.pyramid[pLvl], canvas.pyramid[pLvl + 1],
                                                              filledMap.pyramid[pLvl + 1])
...
def getNeighbourhood(pyramid, pyramidLevel, coord, parms, mirror_hor=False, mirror_vert=False):
    ...
    if pyramidLevel - 1 >= 0:
        Nparent = getSingleLevelNeighbourhood(pyramid, pyramidLevel - 1, coord, parms["parent_kernel_size"],
                                              mode='parent')
    else:
